import { base } from "$app/paths";

const resourceLinks = [
	{
		title: "Publications",
		content: [
			{
				subHeading: "Academic Articles and Books",
				content: [
					{
						text: "Gravelle, Timothy B., \"Friends, Neighbours, Townspeople and Parties\" in Canadian Journal of Political Science/ Revue canadienne de science politique, vol. 51, no. 3, (2018): 643-664. Really good overview of the communities of Muslims who feel belonging.",
						link: "https://www.cambridge.org/core/journals/canadian-journal-of-political-science-revue-canadienne-de-science-politique/article/friends-neighbours-townspeople-and-parties-explaining-canadian-attitudes-toward-muslims/1AE896559899C243B8A6927F4147F55B",
					},
					{
						text: "Hirji, Faiza. \"But where are you really from? South Asians and Muslims in Canada\" in Dreaming in Canadian: South Asian Youth, Bollywood, and Belonging (UBC Press, 2010).",
						link: `${base}/resources/Dreaming_in_Canadian.pdf`,
					},
					{
						text: "Maclean, Derryl N., \"Religion, Ethnicity and the Double Diaspora of Asian Muslims\" in Devries, Larry, Baker, Don and Overmyer, Dan. (eds.) Asian Religions in British Columbia (UBC Press, 2010).",
						link: "https://www.ubcpress.ca/asian-religions-in-british-columbia",
					},
					{
						text: "Mamdani, Mahmood. Good Muslim, bad Muslim: America, the Cold War and the roots of terror (Random House, 2004).",
						link: "https://archive.org/details/goodmuslimbadmus0000mamd/page/n9/mode/2up",
					},
					{
						text: "Nagra, Baljir. Securitized Citizens: Canadian Muslims' Experiences of Race Relations and Identity Formation Post-9/11 (University of Toronto Press, 2017)",
						link: "https://www.jstor.org/stable/10.3138/j.ctv1n35brg",
					},
					{
						text: "Odartey-Wellington, Felix. \"Racial Profiling and Moral Panic: Operation Thread and the Al-Qaeda Sleeper Cell that Never Was\" in Global Media Journal—Canadian Edition, 2: 2 (2009): pp. 25-40.",
						link: "https://www.researchgate.net/publication/40765492_Racial_Profiling_and_Moral_Panic_Operation_Thread_and_the_Al-Qaeda_Sleeper_Cell_that_Never_Was",
					},
					{
						text: "Roach, Kent., \"Canada Responds: Border and Human Security\" in The 9/11 Effect: Comparative Counter-Terrorism (Cambridge, 2012).",
						link: "https://www.cambridge.org/core/books/the-911-effect/8928404A0FEC0B2A22C7E3415272BBB6",
					},
					{
						text: "Said, Edward. \"There are many Islams\"",
						link: "https://polity.lk/wp-content/uploads/2024/09/Pravada-7.56-there-are-many-islams.pdf",
					},
					{
						text: "Said, Edward. Orientalism. (New York: Vintage, 1979).",
						link: "https://monoskop.org/images/4/4e/Said_Edward_Orientalism_1979.pdf",
					},
				],
			},
			{
				subHeading: "Self-Publishing (History)",
				content: [
					{
						text: "Habib, Naiyer and Habib, Mahlwaqa Naushaba. History of the Early Muslims of Regina, Saskatchewan, and their organizations, Islamic Association, Canadian Council of Muslim Women, Muslims for Peace and Justice: A Cultural Integration (Trafford Publishing, 2023).",
						link: "https://www.prweb.com/releases/video_follows_release_of_new_book_history_of_the_muslims_of_regina_saskatchewan_and_their_organizations_/prweb13107368.htm",
					},
				],
			},
		],
	},
	{
		title: "Biography and Autobiography",
		content: [
			{
				subHeading: "Personal Accounts",
				content: [
					{
						text: "Habib, Naiyer and Habib, Mahlwaqa Naushaba. Journey on Parallel Roads: Autobiography and Memoir. (Victoria: Friesen Press, 2019).",
						link: "https://www.amazon.ca/Journey-Parallel-Roads-Autobiography-Memoir/dp/1525526715",
					},
				],
			},
		],
	},
	{
		title: "Unpublished Theses and Dissertations",
		content: [
			{
				subHeading: "Academic Research",
				content: [
					{
						text: "Barclay, Victoria Marissa. The Right to Housing is Social Phenomenon: lessons from racialized women about community, safety & Landlords, MA diss., (UBC, 2023).",
						link: "https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0433826",
					},
					{
						text: "Kraft, Molly. \"Unbelongings: Muslim Women in Multicultural Canada\", MA diss., Geography, *UBC, 2012).",
						link: "#",
					},
					{
						text: "Suleman, Zoolfikar KB. \"Silent Adhan: Exploring the Muslim call to prayer in Metro Vancouver. MA, Interdisciplinary Studies (UBC, 2022).",
						link: "https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0421418",
					},
				],
			},
		],
	},
	{
		title: "Websites",
		content: [
			{
				subHeading: "Government and Official Reports",
				content: [
					{
						text: "Brietkreuz, Garry., MP Chair, \"Review of the Findings and Recommendations Arising from the Iacobucci and O'Connor Inquiries\", Report of the Standing Committee on Public Safety and National Security (June, 2009).",
						link: "https://www.ourcommons.ca/Content/Committee/402/SECU/Reports/RP4004074/securp03/securp03-e.pdf",
					},
					{
						text: "\"A Hopeful Path Forward for Canada's Muslim Communities\", Office of Canada's Special Representative on Combatting Islamophobia, 2023-2024 Annual Report.",
						link: "https://www.canada.ca/content/dam/pch/documents/campaigns/combatting-islamophobia-canada/2024-Multi-OSRI-report-eng.pdf",
					},
					{
						text: "\"Proclamation Requesting to Observe January 29 of Every Year...\"",
						link: "https://laws-lois.justice.gc.ca/eng/regulations/SI-2021-17/page-1.html",
					},
				],
			},
			{
				subHeading: "Research and Data",
				content: [
					{
						text: "Kramer, Stephanie and Tong, Yunping. The Religious Composition of the World's Migrants Report, August 19th 2024, Pew Research Center.",
						link: "https://www.pewresearch.org/religion/2024/08/19/the-religious-composition-of-the-worlds-migrants/",
					},
					{
						text: "\"Religious Composition of the World's Migrants, 1990-2020\" Feature, August 19th 2024, Pew Research Center",
						link: "https://www.pewresearch.org/religion/2024/08/19/religious-composition-of-the-worlds-migrants-1990-2020/",
					},
					{
						text: "Kramer, Stephanie and Tong, Yunping. \"4. Muslim migrants around the world\", The Religious Composition of the World's Migrants Report, August 19th 2024, Pew Research Center.",
						link: "https://www.pewresearch.org/religion/2024/08/19/muslim-migrants-around-the-world/",
						},
					{
						text: "Muslims in Canada: Results from the 2021 Census - Statistics Canada",
						link: "https://www150.statcan.gc.ca/n1/pub/11-627-m/11-627-m2024058-eng.htm",
					},
					{
						text: "Regions, Regionalism and Regional Differences in Canada - Research Study",
						link: "https://www.researchgate.net/publication/259433093_Regions_Regionalism_and_Regional_Differences_in_Canada",
					},
				],
			},
			{
				subHeading: "Community and Educational Resources",
				content: [
					{
						text: "Simerg. \"Award winning Ismaili journalist Salim Jiwa passes away in Vancouver, aged 73.\" 24th October 2024.",
						link: "https://simerg.com/2024/10/24/award-winning-ismaili-journalist-salim-jiwa-passes-away-in-vancouver-aged-73/",
					},
					{
						text: "Syed, Himy. 30 Masjids in 30 Days of Ramadan 2024/1445",
						link: "http://30masjids.ca/",
					},
					{
						text: "The Ismaili Community",
						link: "https://the.ismaili/ca/news/the-ismaili-community",
					},
					{
						text: "Uganda Collection at Carleton University.",
						link: "https://carleton.ca/uganda-collection/about-us/",
						},
					{
						text: "The Pioneers: Exploring the History of South Asian Muslims in Canada - Heritage Mississauga",
						link: "https://heritagemississauga.com/the-pioneers-exploring-the-history-of-south-asian-muslims-in-canada/",
					},
					{
						text: "Muslims in Canada Archives - A digital collection preserving Muslim Canadian history",
						link: "https://www.muslimsincanadaarchives.ca/en/",
					},
				],
			},
		],
	},
	{
		title: "Associations",
		content: [
			{
				subHeading: "Religious Organizations",
				content: [
					{
						text: "British Columbia Muslim Association - Owns and operates 8 mosques in BC. Also organizes schools and classes that focus on Islamic teachings.",
						link: "http://bcmuslims.com/listing/bc-muslim-association/",
					},
					{
						text: "Shia Muslim Community of British Columbia",
						link: "http://bcmuslims.com/listing/shia-muslim-community-of-bc/",
					},
					{
						text: "Ismaili Muslim Community of British Columbia",
						link: "#",
					},
					{
						text: "Ahmadiyya Muslim Community of British Columbia",
						link: "#",
					},
					{
						text: "Al Ihsan Educational Foundation - Focuses on Islamic and Quranic education.",
						link: "https://ihsan.ca/about/",
					},
				],
			},
			{
				subHeading: "Cultural and Community Organizations",
				content: [
					{
						text: "Pakistan Canada Association - Founded in 1963 and owner/manager of the Al-Jamia Masjid, the first mosque in British Columbia established in 1965.",
						link: "https://pakcanbc.com/about-us",
					},
					{
						text: "Pakistan Canada Cultural Association",
						link: "http://bcmuslims.com/listings/prayer-space/",
					},
					{
						text: "BC-Sri Lanka Muslim Association - Based in Vancouver and the Lower Mainland.",
						link: "#",
					},
					{
						text: "Muslim Business Council of BC",
						link: "https://muslimlink.ca/directory/vancouver/muslim-services/organizations/5588-Muslim-Business-Council-of-BC?city=vancouver",
					},
				],
			},
			{
				subHeading: "National Organizations with BC Presence",
				content: [
					{
						text: "National Council of Canadian Muslims - Has a Vancouver based lawyer and the first board chair from Western Canada.",
						link: "https://www.nccm.ca/",
					},
					{
						text: "Canada Sri Lanka Muslim Association",
						link: "https://csmaontario.ca/",
					},
				],
			},
		],
	},
];

export { resourceLinks };